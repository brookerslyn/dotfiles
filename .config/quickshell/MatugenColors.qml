...
Item {
    ...
    Process {
        id: reader
        command: ["cat", "/home/brook/.cache/matugen/qs_colors.json"]
        running: true
        onExited: {
            let txt = reader.stdout.text ? reader.stdout.text.trim() : ""
            if (txt !== "") {
                try {
    ...
    Process {
        id: watcher
        command: ["inotifywait", "-qq", "-e", "close_write,modify", "/home/brook/.cache/matugen/qs_colors.json"]
        running: true
        onExited: root.loadColors()
    }
}
