...
PT="timeout 1.5 playerctl --ignore-player=firefox,chromium,org.webkit.app"
...
# --- 2. CHECK STATUS ---
# Find the first Playing or Paused player (preferring Playing)
ACTIVE_PLAYER=""
STATUS="Stopped"
for player in $(timeout 1.5 playerctl --ignore-player=firefox,chromium,org.webkit.app -l 2>/dev/null); do
    s=$(timeout 1.5 playerctl -p "$player" status 2>/dev/null)
    if [ "$s" = "Playing" ]; then
        ACTIVE_PLAYER="$player"
        STATUS="Playing"
        break
    elif [ "$s" = "Paused" ] && [ -z "$ACTIVE_PLAYER" ]; then
        ACTIVE_PLAYER="$player"
        STATUS="Paused"
    fi
done

# Override PT to use the chosen player
...
if [ "$STATUS" = "Playing" ] || [ "$STATUS" = "Paused" ]; then
    ...
    if [ -f "$finalArt" ] && [ -s "$finalArt" ]; then
        ...
        if [ ! -f "$lockFile" ] && [ -n "$rawUrl" ]; then
            touch "$lockFile"
            # We redirect standard output/error to /dev/null
    ...
    # Use native WirePlumber (wpctl) instead of pactl to prevent D-Bus deadlocks
